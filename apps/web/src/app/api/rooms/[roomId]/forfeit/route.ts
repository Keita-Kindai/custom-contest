import { forfeitRequestSchema } from "@custom-contest/contracts";
import { forfeit } from "@custom-contest/domain";
...
export async function POST(request: Request, { params }: RoomRouteContext) {
  const input = await parseBody(request, forfeitRequestSchema);
  ...
  const result = forfeit(access.room, input.participantKey, commandContext());
