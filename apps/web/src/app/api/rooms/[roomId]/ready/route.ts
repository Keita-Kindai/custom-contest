import { readyRequestSchema } from "@custom-contest/contracts";
import { setReady } from "@custom-contest/domain";
...
export async function POST(request: Request, { params }: RoomRouteContext) {
  const input = await parseBody(request, readyRequestSchema);
  ...
  const result = setReady(access.room, input.participantKey, input.ready, commandContext());
