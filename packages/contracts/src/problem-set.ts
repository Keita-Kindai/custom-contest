 ...
 * 1セットに入れられる問題数の上限。
 *
 * 50では狭い。1つの色帯だけを集めたセット（緑だけ、水だけ）は普通に80〜100問になるので、
 * 作りたいものがそのまま作れない。倍の余裕を見て200にしてある。
 *
 * これ以上は上げない。上限は`MAX_SETS_PER_USER`との掛け算で効き、1アカウントが
 * `problem_set_items`へ書ける行数の最悪値がそれになる。200×200で4万行。
 * 片方を上げるなら、もう片方を下げるかどうかを一緒に決めること。
 *
 * 一括追加は1ページ最大100件なので、上限まで埋めるにはページを跨いで足すことになる。
 * 追加側は残り枠（`room`）だけを受け取って打ち切る。
 */
export const MAX_PROBLEMS_PER_SET = 200;
